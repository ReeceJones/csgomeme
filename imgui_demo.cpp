...
void ImGui::ShowTestWindow(bool* p_open)
{
	...
	if (ImGui::CollapsingHeader("Widgets"))
	{
		if (ImGui::TreeNode("Trees"))
		{
			...
			if (ImGui::TreeNode("Advanced, with Selectable nodes"))
			{
				...
				static int selection_mask = (1 << 2);                 int node_clicked = -1;                                ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing, ImGui::GetFontSize() * 3);                 for (int i = 0; i < 6; i++)
				{
				...
				if (node_clicked != -1)
				{
					if (ImGui::GetIO().KeyCtrl)
						selection_mask ^= (1 << node_clicked);                              else                         selection_mask = (1 << node_clicked);
				}
				ImGui::PopStyleVar();
				if (align_label_with_current_x_position)
		...
		if (ImGui::TreeNode("Images"))
		{
			...
			{
				ImGui::PushID(i);
				int frame_padding = -1 + i;                     if (ImGui::ImageButton(tex_id, ImVec2(32, 32), ImVec2(0, 0), ImVec2(32.0f / tex_w, 32 / tex_h), frame_padding, ImColor(0, 0, 0, 255)))
					pressed_count += 1;
				ImGui::PopID();
				ImGui::SameLine();
			}
	...
	if (ImGui::CollapsingHeader("Graphs widgets"))
	{
		...
		if (animate)
		{
			static float refresh_time = ImGui::GetTime();             for (; ImGui::GetTime() > refresh_time + 1.0f / 60.0f; refresh_time += 1.0f / 60.0f)
			{
	...
	if (ImGui::CollapsingHeader("Layout"))
	{
		...
		if (ImGui::TreeNode("Text Baseline Alignment"))
		{
			...
			ImGui::AlignFirstTextHeightToWidgets();             ImGui::Text("Text aligned to Widget"); ImGui::SameLine();
			...
			ImGui::AlignFirstTextHeightToWidgets();                     bool node_open = ImGui::TreeNode("Node##2");              ImGui::SameLine(0.0f, spacing); ImGui::Button("Button##2");
		...
		if (ImGui::TreeNode("Horizontal Scrolling"))
		{
			...
			{
				ImGui::BeginChild("scrolling");                 ImGui::SetScrollX(ImGui::GetScrollX() + scroll_x_delta);
				ImGui::End();
			}
			ImGui::TreePop();
		}
	...
	if (ImGui::CollapsingHeader("Columns"))
	{
		...
		{
			ImGui::Text("Without border:");
			ImGui::Columns(3, "mycolumns3", false);              ImGui::Separator();
			for (int n = 0; n < 14; n++)
			{
				char label[32];
			...
			ImGui::Text("With border:");
			ImGui::Columns(4, "mycolumns");             ImGui::Separator();
		...
		if (ImGui::TreeNode("Borders"))
		{
			...
			for (int i = 0; i < 8; i++)
			{
				ImGui::Text("%c%c%c", ((char)(0xc86 + 4818 - 0x1ef7)) + i, ((char)(0xe9d + 3498 - 0x1be6)) + i, ((char)(0x6f7 + 7887 - 0x2565)) + i);
				ImGui::NextColumn();
			}
			ImGui::Columns(1);
			if (h_borders) ImGui::Separator();
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
	...
	const ImGuiStyle default_style;     if (ImGui::Button("Revert Style"))
	...
	if (ImGui::TreeNode("Rendering"))
	{
		...
		ImGui::DragFloat("Global Alpha", &style.Alpha, 0.005f, 0.20f, 1.0f, "%.2f");         ImGui::PopItemWidth();
		ImGui::TreePop();
	}
	...
	if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
	{
		...
		for (int i = 0; i < atlas->Fonts.Size; i++)
		{
			...
			{
				ImGui::DragFloat("font scale", &font->Scale, 0.005f, 0.3f, 2.0f, "%.1f");                             ImGui::Text("Ascent: %f, Descent: %f, Height: %f", font->Ascent, font->Descent, font->Ascent - font->Descent);
		...
		ImGui::DragFloat("this window scale", &window_scale, 0.005f, 0.3f, 2.0f, "%.1f");                      ImGui::DragFloat("global scale", &ImGui::GetIO().FontGlobalScale, 0.005f, 0.3f, 2.0f, "%.1f");         ImGui::PopItemWidth();
...
static void ShowExampleAppMainMenuBar()
{
	if (ImGui::BeginMainMenuBar())
	{
		...
		if (ImGui::BeginMenu("Edit"))
		{
			...
			if (ImGui::MenuItem("Redo", "CTRL+Y", false, false)) {}              ImGui::Separator();
...
static void ShowExampleAppAutoResize(bool* p_open)
{
	...
	for (int i = 0; i < lines; i++)
		ImGui::Text("%*sThis is line %d", i * 4, "", i);     ImGui::End();
}
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
	...
	static int type = 0;
	if (type == 0) ImGui::SetNextWindowSizeConstraints(ImVec2(-1, 0), ImVec2(-1, FLT_MAX));          if (type == 1) ImGui::SetNextWindowSizeConstraints(ImVec2(0, -1), ImVec2(FLT_MAX, -1));          if (type == 2) ImGui::SetNextWindowSizeConstraints(ImVec2(100, 100), ImVec2(FLT_MAX, FLT_MAX));     if (type == 3) ImGui::SetNextWindowSizeConstraints(ImVec2(300, 0), ImVec2(400, FLT_MAX));         if (type == 4) ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX), CustomConstraints::Square);              if (type == 5) ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX), CustomConstraints::Step, (void*)100);
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
	...
	ImGui::Separator();
	{
		...
		ImVec2 canvas_pos = ImGui::GetCursorScreenPos();                    ImVec2 canvas_size = ImGui::GetContentRegionAvail();                if (canvas_size.x < 50.0f) canvas_size.x = 50.0f;
		...
		draw_list->PushClipRect(canvas_pos, ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y));              for (int i = 0; i < points.Size - 1; i += 2)
...
struct ExampleAppConsole
{
	...
	int                   HistoryPos;        ImVector<const char*> Commands;
	...
	ExampleAppConsole()
	{
		...
		Commands.push_back("CLASSIFY");          AddLog("Welcome to ImGui!");
	}
	~ExampleAppConsole()
	{
		ClearLog();
	...
	void    Draw(const char* title, bool* p_open)
	{
		...
		ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(4, 1));         for (int i = 0; i < Items.Size; i++)
		{
			const char* item = Items[i];
			if (!filter.PassFilter(item))
				continue;
			ImVec4 col = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);             if (strstr(item, "[error]")) col = ImColor(1.0f, 0.4f, 0.4f, 1.0f);
		...
		if (ImGui::InputText("Input", InputBuf, IM_ARRAYSIZE(InputBuf), ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory, &TextEditCallbackStub, (void*)this))
		{
			...
			while (input_end > InputBuf && input_end[-1] == ((char)(0x900 + 6801 - 0x2371))) input_end--; *input_end = 0;
			if (InputBuf[0])
	...
	int     TextEditCallback(ImGuiTextEditCallbackData* data)
	{
		...
		case ImGuiInputTextFlags_CallbackCompletion:
		{
			...
			while (word_start > data->Buf)
			{
				...
				if (c == ((char)(0x1d2 + 8584 - 0x233a)) || c == '\t' || c == ((char)(0x1085 + 862 - 0x13b7)) || c == ((char)(0xb20 + 772 - 0xde9)))
					break;
				word_start--;
			}
...
struct ExampleAppLog
{
	...
	ImVector<int>       LineOffsets;            bool                ScrollToBottom;
...
static void ShowExampleAppLayout(bool* p_open)
{
	...
	if (ImGui::Begin("Example: Layout", p_open, ImGuiWindowFlags_MenuBar))
	{
		...
		ImGui::BeginGroup();
		ImGui::BeginChild("item view", ImVec2(0, -ImGui::GetItemsLineHeightWithSpacing()));                 ImGui::Text("MyObject: %d", selected);
		ImGui::Separator();
...
static void ShowExampleAppPropertyEditor(bool* p_open)
{
	...
	struct funcs
	{
		...
		{
			ImGui::PushID(uid);                                  ImGui::AlignFirstTextHeightToWidgets();              bool node_open = ImGui::TreeNode("Object", "%s_%u", prefix, uid);
			ImGui::NextColumn();
			ImGui::AlignFirstTextHeightToWidgets();
