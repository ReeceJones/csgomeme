...
public:
	...
	inline bool HookProp(const char *tableName, const char *propName, RecvVarProxyFn fun)
	{
		RecvProp *recvProp = 0;
...
private:
	inline int Get_Prop(const char *tableName, const char *propName, RecvProp **prop = 0)
	{
	...
	inline int Get_Prop(RecvTable *recvTable, const char *propName, RecvProp **prop = 0)
	{
		int extraOffset = 0;
	...
	inline RecvTable *GetTable(const char *tableName)
	{
		if (m_tables.empty())
			return 0;
