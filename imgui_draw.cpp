...
void ImDrawList::ClearFreeMemory()
{
	...
	{
		if (i == 0) memset(&_Channels[0], 0, sizeof(_Channels[0]));          _Channels[i].CmdBuffer.clear();
...
void ImDrawList::ChannelsSetCurrent(int idx)
{
	...
	memcpy(&_Channels.Data[_ChannelsCurrent].CmdBuffer, &CmdBuffer, sizeof(CmdBuffer));     memcpy(&_Channels.Data[_ChannelsCurrent].IdxBuffer, &IdxBuffer, sizeof(IdxBuffer));
...
void ImDrawList::AddPolyline(const ImVec2* points, const int points_count, ImU32 col, bool closed, float thickness, bool anti_aliased)
{
	...
	else
	{
		...
		const int vtx_count = count * 4;              PrimReserve(idx_count, vtx_count);
...
void ImDrawList::AddConvexPolyFilled(const ImVec2* points, const int points_count, ImU32 col, bool anti_aliased)
{
	...
	if (anti_aliased)
	{
		...
		for (int i0 = points_count - 1, i1 = 0; i1 < points_count; i0 = i1++)
		{
			...
			_VtxWritePtr[0].pos = (points[i1] - dm); _VtxWritePtr[0].uv = uv; _VtxWritePtr[0].col = col;                    _VtxWritePtr[1].pos = (points[i1] + dm); _VtxWritePtr[1].uv = uv; _VtxWritePtr[1].col = col_trans;              _VtxWritePtr += 2;
...
static void         Decode85(const unsigned char* src, unsigned char* dst)
{
	while (*src)
	{
		...
		dst[0] = ((tmp >> 0) & 0xff); dst[1] = ((tmp >> 8) & 0xff); dst[2] = ((tmp >> 16) & 0xff); dst[3] = ((tmp >> 24) & 0xff);           src += 5;
		dst += 4;
	}
}
...
ImFont* ImFontAtlas::AddFontFromFileTTF(const char* filename, float size_pixels, const ImFontConfig* font_cfg_template, const ImWchar* glyph_ranges)
{
	...
	{
		const char* p;
		for (p = filename + strlen(filename); p > filename && p[-1] != ((char)(0x12c + 2640 - 0xb4d)) && p[-1] != '\\'; p--) {}
...
bool    ImFontAtlas::Build()
{
	...
	memset(buf_rects, 0, total_glyph_count * sizeof(stbrp_rect));                  memset(buf_ranges, 0, total_glyph_range_count * sizeof(stbtt_pack_range));
	...
	for (int input_i = 0; input_i < ConfigData.Size; input_i++)
	{
		...
		dst_font->FallbackGlyph = NULL;         for (int i = 0; i < tmp.RangesCount; i++)
		{
			stbtt_pack_range& range = tmp.Ranges[i];
			for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1)
			{
				...
				glyph.XAdvance = (pc.xadvance + cfg.GlyphExtraSpacing.x);                  if (cfg.PixelSnapH)
	...
	else if (pass == 1)
	{
		...
		for (int y = 0, n = 0; y < TEX_DATA_H; y++)
			for (int x = 0; x < TEX_DATA_W; x++, n++)
			{
				...
				TexPixelsAlpha8[offset0] = texture_data[n] == ((char)(0xf1c + 4780 - 0x219a)) ? 0xff : 0x00;
				TexPixelsAlpha8[offset1] = texture_data[n] == ((char)(0x10c7 + 393 - 0x11f8)) ? 0xff : 0x00;
			}
		...
		{
			{ ImVec2(0,3),  ImVec2(12,19), ImVec2(0, 0) },{ ImVec2(13,0), ImVec2(7,16),  ImVec2(4, 8) },{ ImVec2(31,0), ImVec2(23,23), ImVec2(11,11) },{ ImVec2(21,0), ImVec2(9,23), ImVec2(5,11) },{ ImVec2(55,18),ImVec2(23, 9), ImVec2(11, 5) },{ ImVec2(73,0), ImVec2(17,17), ImVec2(9, 9) },{ ImVec2(55,0), ImVec2(17,17), ImVec2(9, 9) }, };
...
const ImWchar*  ImFontAtlas::GetGlyphRangesKorean()
{
	static const ImWchar ranges[] =
	{
		0x0020, 0x00ff,         0x3131, 0x3163,         0xac00, 0xd79d,         0,
	};
	return &ranges[0];
}
...
const ImWchar*  ImFontAtlas::GetGlyphRangesChinese()
{
	static const ImWchar ranges[] =
	{
		0x0020, 0x00ff,         0x3000, 0x30ff,         0x31f0, 0x31ff,         0xff00, 0xffef,         0x4e00, 0x9faf,         0,
	};
	return &ranges[0];
}
...
const ImWchar*  ImFontAtlas::GetGlyphRangesJapanese()
{
	...
	{
		0x0020, 0x00ff,         0x3000, 0x30ff,         0x31f0, 0x31ff,         0xff00, 0xffef, };
...
{
	Scale = 1.0f;
	FallbackChar = (ImWchar)((char)(0x1661 + 3483 - 0x23bd));
	Clear();
}
...
void ImFont::BuildLookupTable()
{
	...
	IM_ASSERT(Glyphs.Size < 0xffff);     IndexXAdvance.clear();
	IndexLookup.clear();
	...
	if (FindGlyph((unsigned short)((char)(0xd73 + 3797 - 0x1c28))))
	{
		if (Glyphs.back().Codepoint != '\t')               Glyphs.resize(Glyphs.Size + 1);
		ImFont::Glyph& tab_glyph = Glyphs.back();
		tab_glyph = *FindGlyph((unsigned short)((char)(0x3cd + 7977 - 0x22d6)));
		tab_glyph.Codepoint = '\t';
...
{
	IM_ASSERT(IndexLookup.Size > 0);        int index_size = IndexLookup.Size;

	if (dst < index_size && IndexLookup.Data[dst] == (unsigned short)-1 && !overwrite_dst)         return;
...
const char* ImFont::CalcWordWrapPositionA(float scale, const char* text, const char* text_end, float wrap_width) const
{
	...
	while (s < text_end)
	{
		...
		else
		{
			...
			inside_word = !(c == ((char)(0x1d2 + 8613 - 0x2349)) || c == ((char)(0xfe7 + 5827 - 0x267e)) || c == ((char)(0x2497 + 68 - 0x24a0)) || c == ((char)(0x1b60 + 1520 - 0x212f)) || c == ((char)(0x6d4 + 5930 - 0x1dbf)) || c == '\"');
		}
...
{
	if (c == ((char)(0xa0b + 1270 - 0xee1)) || c == '\t' || c == '\n' || c == '\r')         return;
...
static unsigned int stb_decompress(unsigned char *output, unsigned char *i, unsigned int length)
{
	...
	if (stb__in4(4) != 0)          return 0;     olen = stb_decompress_length(i);
	stb__barrier2 = i;
