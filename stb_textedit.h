...
typedef struct
{
	...
	unsigned char cursor_at_end_of_line;    unsigned char initialized;
	unsigned char has_preferred_x;
	unsigned char single_line;
	unsigned char padding1, padding2, padding3;
	float preferred_x;    StbUndoState undostate;
} STB_TexteditState;
...
typedef struct
{
	float x0, x1;                float baseline_y_delta;     float ymin, ymax;            int num_chars;
} StbTexteditRow;
#endif 
...
typedef struct
{
	float x, y;       float height;    int first_char, length;    int prev_first;
} StbFindState;
...
{
	if (STB_TEXT_HAS_SELECTION(state)) {
		stb_textedit_delete_selection(str, state);       state->has_preferred_x = 0;
		return 1;
	}
	return 0;
}
	...
	default: {
		...
		if (c > 0) {
			...
			if (state->insert_mode && !STB_TEXT_HAS_SELECTION(state) && state->cursor < STB_TEXTEDIT_STRINGLEN(str)) {
				...
				stb_textedit_delete_selection(str, state);                if (STB_TEXTEDIT_INSERTCHARS(str, state->cursor, &ch, 1)) {
...
static void stb_textedit_discard_undo(StbUndoState *state)
{
	if (state->undo_point > 0) {
		if (state->undo_rec[0].char_storage >= 0) {
			...
			state->undo_char_point = state->undo_char_point - (short)n;           STB_TEXTEDIT_memmove(state->undo_char, state->undo_char + n, (size_t)((size_t)state->undo_char_point * sizeof(STB_TEXTEDIT_CHARTYPE)));
...
static void stb_textedit_discard_redo(StbUndoState *state)
{
	...
	if (state->redo_point <= k) {
		if (state->undo_rec[k].char_storage >= 0) {
			...
			state->redo_char_point = state->redo_char_point + (short)n;          STB_TEXTEDIT_memmove(state->undo_char + state->redo_char_point, state->undo_char + state->redo_char_point - n, (size_t)((size_t)(STB_TEXTEDIT_UNDOCHARCOUNT - state->redo_char_point) * sizeof(STB_TEXTEDIT_CHARTYPE)));
